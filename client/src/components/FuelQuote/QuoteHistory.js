...
const QuoteHistory = () => {
  ...
  useEffect(() => {
    if (!username) {
      setMessage(
        "Our system detected that you are not logged in yet. Redirecting to the login screen ..."
      );
      setTimeout(() => {
        ...
        <Paper sx={{ width: "97%", margin: "auto", overflow: "hidden" }}>
          <TableContainer>
            <Table sx={{ minWidth: 700 }} aria-label="customized table">
                ...
                {quotes
                  ...
                  .map((quote) => (
                    <StyledTableRow key={quote.order_id}>
                      <StyledTableCell align="center">
                        {formatDate(quote.purchase_date)}
                      </StyledTableCell>
                      <StyledTableCell align="center">
                        {quote.gallons_amount}
                      </StyledTableCell>
                      <StyledTableCell align="center">
                      ...
                      <StyledTableCell align="center">
                        {parseFloat(quote.unit_cost).toFixed(2)}
                      </StyledTableCell>
                      <StyledTableCell align="center">
              ...
              {quotes.length === 0 && (
                <caption className="text-center ps-2 m-auto">
                  No History
                </caption>
              )}
            </Table>
